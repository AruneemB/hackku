...
Initialize the project, install all dependencies, and configure `.env.local` with real API keys. All teammates must have a working dev environment before other steps begin.
...
`src/types/*.ts` — Define all interfaces before any implementation begins. This is the team contract that allows parallel development. No teammate should write a function without first checking the type for its input/output.
...
Configure Google provider with Gmail scopes (`gmail.compose`, `gmail.readonly`). Store `access_token` + `refresh_token` in the JWT session for Gmail API calls.
...
`src/lib/flights/fairGrid.ts` — Searches all airports within 100 miles x 5-day window.
...
Fair Grid formula: `airports x date_window` combinations, sorted by price, with Saturday-night savings calculated per result.
...
`/api/trips/[id]/bundles` — Gemini analyzes flights + hotels and returns 3 ranked bundles.
...
`src/components/travel/CrisisAlert.tsx` — Full-screen alert with alternative flight. Crisis triggers when `delayMinutes` exceeds the connection buffer. If rebooking is over budget, Gemini drafts an exception request email automatically.
...
Gmail scan for missed receipts -> Gemini extracts -> appended to trip. Gemini generates expense report draft. Trip status updated to `archived`. Privacy summary screen (Frame 16) displayed.
...
| `flight_status` | TimeSeries | Live flight updates (Frames 9-11) |
...
All six require no visa for US citizens, which simplifies the demo. Visa requirements are still seeded and looked up via vector search to demonstrate the capability.
...
15. Post-trip: photograph restaurant receipt -> Gemini extracts 43.50 EUR -> $47.23 USD
